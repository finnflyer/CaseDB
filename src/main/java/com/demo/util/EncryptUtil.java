...
public class EncryptUtil {
    ...
    public static void main(String[] args) {
        try {
            String test = "201505130957510755JL";
    ...
    public static String enString(String temp){
        try {
            String key = "9ba45bfd500642328ec03ad8ef1b6e75";// 自定义密钥
            EncryptUtil des = new EncryptUtil(key, "utf-8");
            return des.encode(temp);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return  null;
    }
    public static String deString(String temp){
        try {
            String key = "9ba45bfd500642328ec03ad8ef1b6e75";// 自定义密钥
            EncryptUtil des = new EncryptUtil(key, "utf-8");
            temp = temp.replace(" ","+");
            return des.decode(temp);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return  null;
