...
<html>
  ...
  <body>
  <script type="text/javascript" src="<%=basePath %>js/jquery.tablednd.js"></script>
  <style type="text/css">
  tr.myDragClass td{
...
function changeID(){
    ...
    <div class="row">
		...
		<div class="col-lg-10">
	   <s:form class="form-horizontal" id="editForm" action="phase4/UpdateTestPlan" theme="simple" >
     ...
     <s:hidden name="testPlanInstkey" value="%{testPlanInstkey}"></s:hidden>
	     <table class="table table table-bordered table-striped" id="test" style="table-layout:fixed">
    		...
    		<a href="javascript:void(0);" onclick='deleteRow(this)'>
						<img src="<%=basePath%>image/cancel.png" width="15" height="15">
					</a><s:hidden   name="conList[%{#st.index}].tpOrder"  value='%{#st.index+1}' />	</td>
    		</td>
    			<td style="width:250;word-wrap:break-word;color:darkblue;font-size:120%"><a href='<%=path%>/phase4/ShowTestCaseDetail?testCase.caseinstkey=<s:property  value="%{contentList[#st.index].caseinstkey}" />' >
